...
export function TradeProgressPage() {
  return (
    <BlockBookShell
      title="거래 진행 상태"
      subtitle="현재 어느 단계인지 바로 보이도록 세로 타임라인 중심으로 정리했습니다."
      showBackButton
      actions={
      ...
      <section className="space-y-4">
        <div className="rounded-[32px] border border-[#dbe6f5] bg-white p-5 shadow-card">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-base font-semibold text-[#314158]">Operating System Concepts</div>
              <div className="mt-1 text-sm text-[#7b8ea8]">에스크로 Lock 완료</div>
            </div>
            <div className="rounded-2xl bg-[#eef4ff] px-3 py-2 text-xs font-semibold text-[#5b82df]">진행중</div>
          </div>
                ...
                <div className="flex flex-col items-center">
                  <div
                    className={`flex h-10 w-10 items-center justify-center rounded-full ${
                      item.done ? 'bg-[#bcd4ff] text-[#456fcf]' : 'bg-[#f2f6fc] text-[#9bb0cb]'
                    }`}
                  >
                    {item.done ? <CheckCircle2 className="h-5 w-5" /> : <span className="text-sm">{index + 1}</span>}
                  </div>
                  {index < tradeTimeline.length - 1 ? <div className="mt-2 h-16 w-px bg-[#dbe6f5]" /> : null}
                </div>
